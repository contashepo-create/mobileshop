  ...
  const handlePrintStatement = () => {
    ...
    const rows = data.operations.map((op: any) => {
      ...
      return `<tr>
        ...
        <td style="text-align:center;font-weight:700">${Math.abs(op.Balance).toFixed(2)}</td>
      </tr>`;
    }).join('');
    ...
    <div class="sheet">
      ...
      <div class="party-box">
        <div><div class="label">اسم المورد</div><div class="value">${supplier.Name}</div></div>
        <div><div class="label">الهاتف</div><div class="value">${supplier.Phone || '—'}</div></div>
        <div><div class="label">العنوان</div><div class="value">${supplier.Address || '—'}</div></div>
      ...
      <table>
        ...
        <tr class="totals-row"><td colspan="4" style="text-align:left;font-weight:700">الإجمالي</td>
          <td style="text-align:center;color:#dc2626">${(data.totals?.totalCredit || 0).toFixed(2)}</td>
          <td style="text-align:center;color:#16a34a">${(data.totals?.totalDebit || 0).toFixed(2)}</td>
          <td style="text-align:center">${Math.abs(netBal).toFixed(2)} ${netBal >= 0 ? '(للمورد)' : '(لنا)'}</td>
        </tr>
      </table>
      <div class="summary">
        <div class="summary-item"><span style="font-size:11px;color:#dc2626">إجمالي المستحق</span><div class="num" style="color:#dc2626">${(data.totals?.totalCredit || 0).toFixed(2)}</div></div>
        <div class="summary-item"><span style="font-size:11px;color:#16a34a">إجمالي المدفوع</span><div class="num" style="color:#16a34a">${(data.totals?.totalDebit || 0).toFixed(2)}</div></div>
      ...
      <div class="footer">هذا الكشف معتمد لدى الطرفين</div>
    </div>
    <script>window.print();window.onafterprint=()=>window.close();<\/script>
    </body></html>`;
    printWindow.document.write(html);
